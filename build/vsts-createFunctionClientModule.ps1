...
$parentModule = 'ForestManagement'
if (-not $ModuleName) { $ModuleName = 'ForestManagement.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\ForestManagement'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\ForestManagement.psd1"
$prereqHash = @{
