function Register-FMAccessRule {
	<#
	.SYNOPSIS
	...
	.DESCRIPTION
		Registers a new access rule as a desired state.
		These are then compared with a domain's configuration when executing Test-FMAccessRule.
	...
	.PARAMETER ObjectCategory
		...
		Categories are defined using Register-AdcObjectCategory.
	...
	.PARAMETER NoFixConfig
		By default, Test-FMAccessRule will generate a "FixConfig" result for accessrules that have been explicitly defined but are also part of the Schema Default permissions.
	...
	.EXAMPLE
		PS C:\> Register-FMAccessRule -ObjectCategory DomainControllers -Identity '%DomainName%\Domain Admins' -ActiveDirectoryRights GenericAll
		...
		switch ($PSCmdlet.ParameterSetName) {
			'Path' {
				...
				$script:accessRules[$Path] += [PSCustomObject]@{
					PSTypeName            = 'ForestManagement.AccessRule'
					Path                  = $Path
