function Register-FMAcl {
	...
	.DESCRIPTION
		Registers an active directory acl.
		This acl will be maintained as configured during Invoke-FMAcl.
	...
	.EXAMPLE
		PS C:\> Get-Content .\groups.json | ConvertFrom-Json | Write-Output | Register-FMAcl
		...
		switch ($PSCmdlet.ParameterSetName) {
			'path' {
				$script:acls[$Path] = [PSCustomObject]@{
					PSTypeName    = 'ForestManagement.Acl'
					Path          = $Path
					Owner         = $Owner
