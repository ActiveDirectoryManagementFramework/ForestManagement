	...
	begin {
		...
		function New-Change {
			...
			$changeItem = [PSCustomObject]@{
				PSTypeName = 'ForestManagement.Acl.Change'
				Type       = $Type
				Old        = $OldValue
				New        = $NewValue
				Identity   = $Identity
		...
		function Get-ChangeByCategory {
			...
			if ($Category.Owner -match '^__.+__$') {
				$simpleOwner = $false
				Invoke-PSFProtectedCommand -ActionString 'Test-FMAcl.ResolveOwner' -ActionStringValues $Category.Owner.Trim('_'),$ADObject.DistinguishedName -Target $Category -ScriptBlock {
	...
	process {
		...
		foreach ($aclDefinition in $script:acls.Values) {
			...
			if (-not (Test-ADObject @parameters -Identity $resolvedPath)) {
				...
				Write-PSFMessage -String 'Test-FMAcl.ADObjectNotFound' -StringValues $resolvedPath -Tag 'panic', 'failed' -Target $aclDefinition
			...
			try { $aclObject = Get-AdsAcl @parameters -Path $resolvedPath -EnableException }
				...
				if ($aclDefinition.Optional) { continue }
				Write-PSFMessage -String 'Test-FMAcl.NoAccess' -StringValues $resolvedPath -Tag 'panic', 'failed' -Target $aclDefinition -ErrorRecord $_
			...
			if ($aclDefinition.Owner -match '^__.+__$') {
				$simpleOwner = $false
				Invoke-PSFProtectedCommand -ActionString 'Test-FMAcl.ResolveOwner' -ActionStringValues $aclDefinition.Owner.Trim('_'),$resolvedPath -Target $aclDefinition -ScriptBlock {
					$ownerSet = Resolve-AdcPrivilegedGroupSet -Type $aclDefinition.Owner.Trim('_') @parameters
				} -Continue -PSCmdlet $PSCmdlet
			}
			else {
