...
<#
	...
	.EXAMPLE
		...
		Resolves the configured filters into searchbases for the targeted forest.
#>
    ...
    process
    {
        ...
        if (($Server -eq $script:contentSearchBases.Server) -and (-not (Compare-Object $script:contentMode.Include $script:contentSearchBases.Include)) -and (-not (Compare-Object $script:contentMode.Exclude $script:contentSearchBases.Exclude))) {
			$script:contentSearchBases.Bases
			return
        }
        ...
        if ($exclude -and -not $include) {
            $include = Resolve-String '%ConfigurationDN%', '%SchemaDN%' | Convert-DistinguishedName
        }
        $allItems = @{}
        foreach ($item in $include) {
			if (-not (Test-ADObject @parameters -Identity $item.Name)) {
				Write-PSFMessage -Level Verbose -String 'Resolve-ContentSearchBase.Include.NotFound' -StringValues $item.Name -Tag notfound, container -Target $Server
				continue
			}
        ...
        foreach ($item in $exclude) {
			if (-not (Test-ADObject @parameters -Identity $item.Name)) {
				Write-PSFMessage -Level Verbose -String 'Resolve-ContentSearchBase.Exclude.NotFound' -StringValues $item.Name -Tag notfound, container -Target $Server
				continue
			}
            $allItems[$item.Name] = $item
        }
